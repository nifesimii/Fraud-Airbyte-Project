...
build: ## Build Airflow Docker image
	...
	@echo $(IMAGE_TAG) > .airflow-image-tag
	@echo "$(GREEN)✓ Built: $(IMAGE_NAME):$(IMAGE_TAG)$(NC)"
	@echo "$(YELLOW)━━━ Build Soda Image (ARM) ━━━$(NC)"
	@docker build -t soda-core-arm:latest -f ../cicd/Dockerfile.soda .
	@echo "$(GREEN)✓ Built: soda-core-arm:latest$(NC)"
...
create-k8s-resources: create-namespace fix-log-permissions ## Create K8s resources
	...
	@kubectl apply -f ../k8s/secrets/airbyte-secrets.yaml
	@kubectl apply -f ../k8s/airflow-logs.yaml
	@kubectl apply -f ../k8s/soda-configmap.yaml
	@echo "$(GREEN)✓ Secrets, storage, and configs created$(NC)"
...
install: pull-postgres update-values setup-helm create-k8s-resources ## Install Airflow
	...
	@helm install airflow apache-airflow/airflow \
		...
		--set scheduler.waitForMigrations.enabled=false \
		--set webserver.waitForMigrations.enabled=false \
		--set api-Server.waitForMigrations.enabled=false \
		--set triggerer.waitForMigrations.enabled=false \
		--set dagProcessor.waitForMigrations.enabled=false \
		--timeout 5m || echo "$(YELLOW)⚠ Helm timeout (expected)$(NC)"
	@echo "$(GREEN)✓ Helm chart installed$(NC)"
...
# 	@echo "$(YELLOW)━━━ Installing Airbyte Secrets ━━━$(NC)"
# 	@kubectl apply -f ../k8s/secrets/airbyte-secrets.yaml
# 	@echo "$(GREEN)✓ Airbyte secrets created$(NC)"
...
wait-scheduler: ## Wait for scheduler to be fully ready
	@echo "$(YELLOW)Waiting for scheduler pod to be ready...$(NC)"
	@for i in $$(seq 1 30); do \
		if kubectl exec -n $(NAMESPACE) deploy/airflow-scheduler -c scheduler -- echo ok >/dev/null 2>&1; then \
			echo "$(GREEN)✓ Scheduler is ready$(NC)"; \
			break; \
		fi; \
		if [ $$i -eq 30 ]; then \
			echo "$(RED)✗ Scheduler not ready after 5 minutes$(NC)"; \
			kubectl get pods -n $(NAMESPACE) -l component=scheduler; \
			exit 1; \
		fi; \
		echo "  Attempt $$i/30 - scheduler not ready, waiting 10s..."; \
		sleep 10; \
	done

add-airbyte-vars: wait-scheduler ## Set Airbyte Cloud connection ID variables
	@echo "$(YELLOW)Setting Airbyte variables...$(NC)"
	@if [ -z "$(AIRBYTE_CONN_POSTGRES_S3)" ] || [ -z "$(AIRBYTE_CONN_MYSQL_S3)" ] || [ -z "$(AIRBYTE_CONN_S3_SNOWFLAKE)" ]; then \
		echo "$(RED)ERROR: Missing Airbyte connection IDs!$(NC)"; \
		echo "$(YELLOW)Required in .env: AIRBYTE_CONN_POSTGRES_S3, AIRBYTE_CONN_MYSQL_S3, AIRBYTE_CONN_S3_SNOWFLAKE$(NC)"; \
...
setup-connections: wait-scheduler add-airbyte-conn add-airbyte-vars add-mysql-conn add-postgres-source-conn setup-snowflake-connection setup-soda-k8s
# ============================================================================
# TESTING
# ============================================================================
...
setup-soda-vars: check-snowflake-creds ## Set and verify Snowflake Airflow variables for Soda
	@echo "$(YELLOW)━━━ Setting Snowflake Variables ━━━$(NC)"
	@kubectl exec -n $(NAMESPACE) deploy/airflow-scheduler -c scheduler -- \
